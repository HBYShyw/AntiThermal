...
public final class OplusManager {
    ...
    private static boolean initStampService() {
        ...
        int times = 3;
        do {
            Log.w(TAG, "Try to get stamp service instance! times = " + times);
    ...
    public static String getOplusRomVersion() {
        String versionConfidential = SystemProperties.get(OplusPropertyList.PROPERTY_VERSION_CONFIDENTIAL, "");
        String buildVersionOplusromConfidential = SystemProperties.get(OplusPropertyList.PROPERTY_BUILD_VERSION_OPLUSROM_CONFIDENTIAL, "");
        if (versionConfidential.equals("true") && !buildVersionOplusromConfidential.equals("")) {
            return buildVersionOplusromConfidential;
        }
        if (versionConfidential.equals("false") && !buildVersionOplusromConfidential.equals("") && SystemProperties.get(OplusPropertyList.PROPERTY_VERSION_UPGRADE_DECRYPT_CONFIG, "").equals("true")) {
            return buildVersionOplusromConfidential;
        }
        return SystemProperties.get("ro.build.version.oplusrom", OplusPropertyList.UNKNOWN);
    }
    ...
    public static int writeLogToPartition(int type, String logstring, String tagString, String issue, int isOnlyAddToDropbox) {
        ...
        String buildTime = "log-buildTime: " + SystemProperties.get("ro.build.version.ota", OplusPhoneInterFaceExt.ICCTYPE_UNKNOWN) + "\n";
        ...
        if (tagString.equals(ANDROID_TAG)) {
            String tagbegin2 = "<android-" + issue2 + "-begin>\n";
            String tagend2 = "\n<android-" + issue2 + "-end>\n";
            tagbegin = tagbegin2;
            tagend = tagend2;
        } else if (tagString.equals(MULTIMEDIA_TAG)) {
            String tagbegin3 = "<multimedia-" + issue2 + "-begin>\n";
            String tagend3 = "\n<multimedia-" + issue2 + "-end>\n";
            tagbegin = tagbegin3;
            tagend = tagend3;
        } else if (tagString.equals(NETWORK_TAG)) {
            String tagbegin4 = "<network-" + issue2 + "-begin>\n";
            String tagend4 = "\n<network-" + issue2 + "-end>\n";
            tagbegin = tagbegin4;
            tagend = tagend4;
        } else if (tagString.equals(KERNEL_TAG)) {
            String tagbegin5 = "<kernel-" + issue2 + "-begin>\n";
            String tagend5 = "\n<kernel-" + issue2 + "-end>\n";
            tagbegin = tagbegin5;
            tagend = tagend5;
        } else if (tagString.equals(CONNECT_TAG)) {
            String tagbegin6 = "<connectivity-" + issue2 + "-begin>\n";
            String tagend6 = "\n<connectivity-" + issue2 + "-end>\n";
            tagbegin = tagbegin6;
            tagend = tagend6;
        } else if (tagString.equals(CAMERA_TAG)) {
            String tagbegin7 = "<camera-" + issue2 + "-begin>\n";
            String tagend7 = "\n<camera-" + issue2 + "-end>\n";
            tagbegin = tagbegin7;
            tagend = tagend7;
        } else {
    ...
    public static void onStamp(String eventId, Map<String, String> logMap) {
        ...
        synchronized (STAMP_SYNC_LOCK) {
            ...
            while (it.hasNext()) {
                StampEvent stampEvent2 = it.next();
                try {
                    IOplusStampService iOplusStampService2 = sStampService;
                    if (iOplusStampService2 != null) {
                        iOplusStampService2.sendOnStampEvent(stampEvent2.eventId, stampEvent2.logMap, Long.parseLong(stampEvent2.timestamp));
                        Log.d(TAG, "send on stamp success " + stampEvent2.eventId);
                    }
                } catch (Exception e2) {
    ...
    public static void onDeleteStampId(String eventId) {
        ...
        try {
            IOplusStampService iOplusStampService = sStampService;
            if (iOplusStampService != null) {
                iOplusStampService.sendDeleteStampId(eventId);
