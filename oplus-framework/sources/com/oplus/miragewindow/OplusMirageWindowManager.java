...
public class OplusMirageWindowManager {
    ...
    public Bundle getTaskInfo(ComponentName name) {
        try {
            return this.mAms.getTaskInfo(name);
        } catch (RemoteException e) {
            Slog.e(TAG, "Failed to get taskInfo of componentName:");
            return null;
        }
    }
    ...
    public int createVirtualDisplayDevice(Bundle bundle) {
        try {
            return this.mAms.createVirtualDisplayDevice(bundle);
        } catch (RemoteException e) {
            Slog.d(TAG, "create virtual display fail");
            return -1;
        }
    }
    ...
    public void releaseVirtualDisplayDevice(int displayId) {
        try {
            this.mAms.releaseVirtualDisplayDevice(displayId);
        } catch (RemoteException e) {
            Slog.d(TAG, "Failed to release failed: remoteException");
        }
    }

    public void moveTaskToDisplay(int taskId, int displayId, boolean top) {
        try {
            this.mAms.moveTaskToDisplay(taskId, displayId, top);
        } catch (RemoteException e) {
            Slog.d(TAG, "Failed to moveTaskToDisplay: remoteException");
        }
    }
}
