...
public class BackupManagerService extends IBackupManager.Stub {
    ...
    private boolean mHasFirstUserUnlockedSinceBoot;
    ...
    public BackupManagerService(Context context) {
        ...
        Slog.d(TAG, "Default backup user id = " + this.mDefaultBackupUserId);
    }
    ...
    protected File getSuppressFileForUser(int i) {
        return new File(UserBackupManagerFiles.getBaseStateDir(i), BACKUP_SUPPRESS_FILENAME);
    }
    ...
    public boolean isUserReadyForBackup(int i) {
        ...
        return this.mUserServices.size() > 0 && this.mUserServices.get(i) != null;
    }
    ...
    public void reportDelayedRestoreResult(String str, List<BackupRestoreEventLogger.DataTypeResult> list) {
        int identifier = Binder.getCallingUserHandle().getIdentifier();
        if (!isUserReadyForBackup(identifier)) {
            Slog.w(TAG, "Returning from reportDelayedRestoreResult as backup for user" + identifier + " is not initialized yet");
            return;
        }
    ...
    public void updateDefaultBackupUserIdIfNeeded() {
        ...
        Slog.i(TAG, "Default backup user changed from " + i + " to " + this.mDefaultBackupUserId);
    }
}
