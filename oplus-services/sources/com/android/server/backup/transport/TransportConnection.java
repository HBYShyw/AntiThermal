...
public class TransportConnection {
    ...
    private void notifyListener(final TransportConnectionListener transportConnectionListener, final BackupTransportClient backupTransportClient, String str) {
    ...
    @GuardedBy({"mStateLock"})
    private void notifyListenersAndClearLocked(BackupTransportClient backupTransportClient) {
    ...
    @GuardedBy({"mStateLock"})
    private void setStateLocked(int i, BackupTransportClient backupTransportClient) {
    ...
    private void onStateTransition(int i, int i2) {
        ...
        if (transitionThroughState != 0) {
            EventLog.writeEvent(EventLogTags.BACKUP_TRANSPORT_LIFECYCLE, flattenToShortString, Integer.valueOf(transitionThroughState == 1 ? 1 : 0));
        }
    ...
    private static class TransportConnectionMonitor implements ServiceConnection {
        ...
        private void referenceLost(String str) {
            try {
                ...
                TransportUtils.log(5, TransportConnection.TAG, "Exception trying to call referenceLost(): " + e.getMessage());
            }
