...
public final class ContentService extends IContentService.Stub {
    ...
    public static IContentServiceExt sContentServiceExt = (IContentServiceExt) ExtLoader.type(IContentServiceExt.class).create();
    ...
    public void registerContentObserver(Uri uri, boolean z, IContentObserver iContentObserver, int i, int i2) {
        ...
        if (checkContentProviderAccess != null) {
            ...
            if (!checkContentProviderAccess.startsWith("Failed to find provider")) {
                Log.w(TAG, "Ignoring content changes for " + uri + " from " + callingUid + ": " + checkContentProviderAccess);
                return;
            }
        }
        int checkUserHandle = sContentServiceExt.checkUserHandle(uri.getAuthority(), handleIncomingUser);
        synchronized (this.mRootNode) {
            ObserverNode observerNode = this.mRootNode;
            observerNode.addObserverLocked(uri, iContentObserver, z, observerNode, callingUid, callingPid, checkUserHandle);
        }
    }
        ...
        for (Uri uri : uriArr) {
            ...
            if (!arrayMap2.containsKey(create)) {
                ...
                if (checkContentProviderAccess != null) {
                    ...
                    if (!checkContentProviderAccess.startsWith("Failed to find provider")) {
                        Log.w(TAG, "Ignoring notify for " + uri + " from " + callingUid + ": " + checkContentProviderAccess);
                    }
                }
    ...
    public static class ObserverCollector {
        ...
        public static class Key {
            ...
            Key(IContentObserver iContentObserver, int i, boolean z, int i2, int i3) {
    ...
    private String getProviderPackageName(Uri uri, int i) {
    ...
    /* JADX INFO: Access modifiers changed from: private */
    @GuardedBy({"mCache"})
    public void invalidateCacheLocked(int i, String str, Uri uri) {
    ...
    private int handleIncomingUser(Uri uri, int i, int i2, int i3, boolean z, int i4) {
        ...
        if (i4 == -1) {
            ...
            if (i4 != UserHandle.getCallingUserId() && checkUriPermission(uri, i, i2, i3, i4) != 0) {
                ...
                if (!z2) {
                    throw new SecurityException("No access to " + uri + ": neither user " + i2 + " nor current process has " + (z ? "android.permission.INTERACT_ACROSS_USERS_FULL or android.permission.INTERACT_ACROSS_USERS" : "android.permission.INTERACT_ACROSS_USERS_FULL"));
                }
            }
        }
        return i4;
    ...
    private void enforceNonFullCrossUserPermission(int i, String str) {
    ...
    private boolean hasAccountAccess(boolean z, Account account, int i) {
        if (account == null) {
            return true;
        }
        if (z && !CompatChanges.isChangeEnabled(ACCOUNT_ACCESS_CHECK_CHANGE_ID, i)) {
            return true;
        }
        long clearCallingIdentity = IContentService.Stub.clearCallingIdentity();
        try {
    ...
    private boolean hasAuthorityAccess(String str, int i, int i2) {
        if (!TextUtils.isEmpty(str) && CompatChanges.isChangeEnabled(AUTHORITY_ACCESS_CHECK_CHANGE_ID, i)) {
    ...
    private int getSyncExemptionAndCleanUpExtrasForCaller(int i, Bundle bundle) {
        ...
        FrameworkStatsLog.write(FrameworkStatsLog.SYNC_EXEMPTION_OCCURRED, i, getProcStateForStatsd(uidProcessState), isUidActive, getRestrictionLevelForStatsd(activityManagerInternal.getRestrictionLevel(i)));
        return 1;
    }
    ...
    public static final class ObserverNode {
        ...
        public class ObserverEntry implements IBinder.DeathRecipient {
            ...
            public ObserverEntry(IContentObserver iContentObserver, boolean z, Object obj, int i, int i2, int i3, Uri uri) {
                ...
                if (linkToDeath == 1000) {
                    ...
                    Slog.wtf(ContentService.TAG, "Observer registered too many times. Leak? cpid=" + this.pid + " cuid=" + this.uid + " cpkg=" + str + " url=" + uri);
                }
            }
        ...
        private void addObserverLocked(Uri uri, int i, IContentObserver iContentObserver, boolean z, Object obj, int i2, int i3, int i4) {
            if (i == countUriSegments(uri)) {
                Iterator<ObserverEntry> it = this.mObservers.iterator();
                while (it.hasNext()) {
                    if (it.next().observer.asBinder() == iContentObserver.asBinder()) {
                        Log.w(ContentService.TAG, "Observer is already registered. Pid is " + Binder.getCallingPid());
                        return;
                    }
                }
                this.mObservers.add(new ObserverEntry(iContentObserver, z, obj, i2, i3, i4, uri));
                ContentService.sContentServiceExt.addProxyBinder(iContentObserver.asBinder(), i2, i3);
                return;
            }
        ...
        public boolean removeObserverLocked(IContentObserver iContentObserver) {
            ...
            while (true) {
                ...
                if (observerEntry.observer.asBinder() == asBinder) {
                    ...
                    ContentService.sContentServiceExt.removeProxyBinder(asBinder, observerEntry.uid);
                    break;
                }
                i2++;
            }
        ...
        private void collectMyObserversLocked(Uri uri, boolean z, IContentObserver iContentObserver, boolean z2, int i, int i2, ObserverCollector observerCollector) {
