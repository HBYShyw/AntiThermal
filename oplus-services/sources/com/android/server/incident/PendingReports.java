...
public class PendingReports {
    ...
    public final class PendingReportRec {
        ...
        Uri getUri() {
            Uri.Builder appendQueryParameter = new Uri.Builder().scheme("content").authority("android.os.IncidentManager").path("/pending").appendQueryParameter("id", Integer.toString(this.id)).appendQueryParameter("pkg", this.callingPackage).appendQueryParameter("flags", Integer.toString(this.flags)).appendQueryParameter("t", Long.toString(this.addedWalltime));
            String str = this.receiverClass;
            if (str != null && str.length() > 0) {
                appendQueryParameter.appendQueryParameter(ParsingPackageUtils.TAG_RECEIVER, this.receiverClass);
            }
    ...
    */
    public void lambda$authorizeReport$0(int i, String str, String str2, String str3, int i2, final IIncidentAuthListener iIncidentAuthListener) {
        boolean z;
        ...
        if (currentUserIfAdmin == -10000 || !isSameProfileGroupUser(userId, currentUserIfAdmin)) {
            Log.w(TAG, "Calling user " + userId + " doesn't belong to the same profile group of the current admin user " + currentUserIfAdmin);
            denyReportBeforeAddingRec(iIncidentAuthListener, str);
            return;
        }
        ...
        if ((Build.IS_USERDEBUG || Build.IS_ENG) && (i2 & 2) != 0) {
            if (this.mPermissionManager.checkPermissionForDataDelivery("android.permission.CAPTURE_CONSENTLESS_BUGREPORT_ON_USERDEBUG_BUILD", new AttributionSource.Builder(i).setPackageName(str).build(), (String) null) == 0) {
                ...
                if (z) {
                    try {
                        Log.d(TAG, "approving consentless report: " + new PendingReportRec(str, str2, str3, i2, iIncidentAuthListener).getUri());
    ...
    public void lambda$cancelAuthorization$1(IIncidentAuthListener iIncidentAuthListener) {
