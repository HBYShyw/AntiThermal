...
public final class BatteryController extends RestrictingController {
    ...
    @GuardedBy({"mLock"})
    private Boolean mLastReportedStatsdBatteryNotLow;

    @GuardedBy({"mLock"})
    private Boolean mLastReportedStatsdStablePower;
    ...
    public void prepareForExecutionLocked(JobStatus jobStatus) {
        if (jobStatus.hasPowerConstraint()) {
            ...
            if (this.mService.getUidBias(jobStatus.getSourceUid()) == 40 && jobStatus.hasPowerConstraint()) {
                if (z) {
    ...
    @GuardedBy({"mLock"})
    private boolean hasTopExemptionLocked(JobStatus jobStatus) {
    ...
    public void maybeReportNewChargingStateLocked() {
        ...
        if (DEBUG) {
            Slog.d(TAG, "maybeReportNewChargingStateLocked: " + isPowerConnected + SliceClientPermissions.SliceAuthority.DELIMITER + z + SliceClientPermissions.SliceAuthority.DELIMITER + isBatteryNotLow);
        }
