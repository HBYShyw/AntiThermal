...
public final class TimeController extends StateController {
    ...
    public void maybeStartTrackingJobLocked(JobStatus jobStatus, JobStatus jobStatus2) {
        ...
        if (jobStatus.hasTimingDelayConstraint() || jobStatus.hasDeadlineConstraint()) {
            ...
            if (this.mService.getWrapper().getExtImpl() == null || !this.mService.getWrapper().getExtImpl().isProxyJob(jobStatus, "maybeStartTrackingJobLocked")) {
    ...
    void checkExpiredDelaysAndResetAlarm() {
        synchronized (this.mLock) {
            ...
            while (it.hasNext()) {
                ...
                if (next != null && next.hasTimingDelayConstraint()) {
    ...
    private void setDelayExpiredAlarmLocked(long j, WorkSource workSource) {
        long maybeAdjustAlarmTime = maybeAdjustAlarmTime(Math.max(j, this.mLastFiredDelayExpiredElapsedMillis + DELAY_COALESCE_TIME_MS));
