...
public class LazyAlarmStore implements AlarmStore {
    ...
    private static final Comparator<Alarm> sDecreasingTimeOrder = Comparator.comparingLong(new ToLongFunction() { // from class: com.android.server.alarm.LazyAlarmStore$$ExternalSyntheticLambda0
    ...
    public ArrayList<Alarm> removePendingAlarms(long j) {
        ...
        for (int size = this.mAlarms.size() - 1; size >= 0; size--) {
            ...
            arrayList.add(alarm);
            if (alarm.wakeup && alarm.getMaxWhenElapsed() <= ALARM_DEADLINE_SLOP + j) {
                z = true;
            }
            if ((alarm.flags & 1) != 0) {
                z2 = true;
