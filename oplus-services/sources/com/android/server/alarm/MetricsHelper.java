...
public class MetricsHelper {
    ...
    public void registerPuller(final Supplier<AlarmStore> supplier) {
        ((StatsManager) this.mContext.getSystemService(StatsManager.class)).setPullAtomCallback(FrameworkStatsLog.PENDING_ALARM_INFO, (StatsManager.PullAtomMetadata) null, BackgroundThread.getExecutor(), new StatsManager.StatsPullAtomCallback() { // from class: com.android.server.alarm.MetricsHelper$$ExternalSyntheticLambda0
    ...
    public static void pushAlarmScheduled(Alarm alarm, int i) {
        FrameworkStatsLog.write(FrameworkStatsLog.ALARM_SCHEDULED, alarm.uid, alarm.windowLength == 0, alarm.wakeup, (alarm.flags & 4) != 0, alarm.alarmClock != null, alarm.repeatInterval != 0, reasonToStatsReason(alarm.mExactAllowReason), AlarmManagerService.isRtc(alarm.type), ActivityManager.processStateAmToProto(i));
    }
