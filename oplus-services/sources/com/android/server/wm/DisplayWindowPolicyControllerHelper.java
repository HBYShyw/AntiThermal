...
public class DisplayWindowPolicyControllerHelper {
    ...
    public boolean canContainActivities(List<ActivityInfo> list, @WindowConfiguration.WindowingMode int i) {
        ...
        if (displayWindowPolicyController == null) {
            for (int size = list.size() - 1; size >= 0; size--) {
                ...
                if (str != null) {
                    Slog.e(TAG, String.format("Activity with requiredDisplayCategory='%s' cannot be displayed on display %d because that display does not have a matching category", str, Integer.valueOf(this.mDisplayContent.mDisplayId)));
                    return false;
                }
            }
            return true;
    ...
    public boolean canActivityBeLaunched(ActivityInfo activityInfo, Intent intent, @WindowConfiguration.WindowingMode int i, int i2, boolean z) {
        ...
        if (displayWindowPolicyController == null) {
            ...
            Slog.e(TAG, String.format("Activity with requiredDisplayCategory='%s' cannot be launched on display %d because that display does not have a matching category", str, Integer.valueOf(this.mDisplayContent.mDisplayId)));
            return false;
        }
