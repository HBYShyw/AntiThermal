...
public class EnergyConsumerSnapshot {
    ...
    public EnergyConsumerDeltaData updateAndGetDelta(EnergyConsumerResult[] energyConsumerResultArr, int i) {
        ...
        String str2 = TAG;
        if (i <= 0) {
            Slog.wtf(TAG, "Unexpected battery voltage (" + i + " mV) when taking energy consumer snapshot");
            return null;
        }
    ...
    public void dump(PrintWriter printWriter) {
        ...
        for (int i2 = 0; i2 < this.mEnergyConsumerSnapshots.size(); i2++) {
            printWriter.println(String.format("    Consumer %d has energy %d uJ at %d mV", Integer.valueOf(this.mEnergyConsumerSnapshots.keyAt(i2)), Long.valueOf(this.mEnergyConsumerSnapshots.valueAt(i2)), Long.valueOf(this.mVoltageSnapshots.valueAt(i2))));
        }
    ...
    */
    private BatteryStats.EnergyConsumerDetails createEnergyConsumerDetails() {
                ...
                if (!z2) {
                    for (int i2 = 0; i2 < this.mEnergyConsumers.size(); i2++) {
                        ...
                        if (valueAt2.type == energyConsumer.type && valueAt2.ordinal != 0) {
                            if (!z) {
                                energyConsumer.name += SliceClientPermissions.SliceAuthority.DELIMITER + valueAt.ordinal;
                            }
                        }
                    }
                }
