...
public class BroadcastSkipPolicy {
    ...
    private String shouldSkipMessage(BroadcastRecord broadcastRecord, ResolveInfo resolveInfo) {
        ...
        if (!broadcastSkipPolicy.mService.validateAssociationAllowedLocked(broadcastRecord.callerPackage, broadcastRecord.callingUid, componentName.getPackageName(), resolveInfo.activityInfo.applicationInfo.uid)) {
            return "Association not allowed: broadcasting " + broadcastDescription(broadcastRecord, componentName);
        }
        if (!broadcastSkipPolicy.mService.mIntentFirewall.checkBroadcast(broadcastRecord.intent, broadcastRecord.callingUid, broadcastRecord.callingPid, broadcastRecord.resolvedType, resolveInfo.activityInfo.applicationInfo.uid)) {
        ...
        if (broadcastRecord.callerInstantApp) {
            ...
            if ((activityInfo4.flags & AudioDevice.OUT_FM) == 0 && broadcastRecord.callingUid != activityInfo4.applicationInfo.uid) {
        ...
        try {
            ...
            return this.mBroadcastSkipPolicyExt.shouldSkipMessage(broadcastRecord, resolveInfo);
    ...
    public boolean disallowBackgroundStart(BroadcastRecord broadcastRecord) {
        return (broadcastRecord.intent.getFlags() & AudioDevice.OUT_IP) != 0 || (broadcastRecord.intent.getComponent() == null && broadcastRecord.intent.getPackage() == null && (broadcastRecord.intent.getFlags() & 16777216) == 0 && !isSignaturePerm(broadcastRecord.requiredPermissions));
    }
    ...
    private String shouldSkipMessage(BroadcastRecord broadcastRecord, BroadcastFilter broadcastFilter) {
        ...
        if (!broadcastSkipPolicy.mService.validateAssociationAllowedLocked(broadcastRecord.callerPackage, broadcastRecord.callingUid, broadcastFilter.packageName, broadcastFilter.owningUid)) {
        ...
        if (!broadcastSkipPolicy.mService.mIntentFirewall.checkBroadcast(broadcastRecord.intent, broadcastRecord.callingUid, broadcastRecord.callingPid, broadcastRecord.resolvedType, broadcastFilter.receiverList.uid)) {
        ...
        if (processRecord == null || processRecord.isKilled() || broadcastFilter.receiverList.app.mErrorState.isCrashing()) {
            if (broadcastFilter.receiverList.app != null) {
                Slog.w(BroadcastQueue.TAG, "Process state: app.isKilled = " + broadcastFilter.receiverList.app.isKilled() + " app.mErrorState.isCrashing = " + broadcastFilter.receiverList.app.mErrorState.isCrashing());
            }
        ...
        if (!((broadcastRecord.intent.getFlags() & AudioDevice.OUT_AUX_LINE) != 0) && broadcastFilter.instantApp && broadcastFilter.receiverList.uid != broadcastRecord.callingUid) {
        ...
        if (broadcastRecord.appOp != -1) {
            if (this.mService.getAppOpsManager().noteOpNoThrow(broadcastRecord.appOp, broadcastFilter.receiverList.uid, broadcastFilter.packageName, broadcastFilter.featureId, str29 + broadcastFilter.receiverId) != 0) {
        ...
        if (!z && ActivityManagerService.checkComponentPermission(null, broadcastRecord.callingPid, i5, broadcastFilter.receiverList.uid, z) != 0) {
        ...
        return this.mBroadcastSkipPolicyExt.shouldSkipMessage(broadcastRecord, broadcastFilter);
    }
    ...
    private boolean noteOpForManifestReceiverInner(int i, BroadcastRecord broadcastRecord, ResolveInfo resolveInfo, ComponentName componentName, String str) {
        ...
        ActivityInfo activityInfo = resolveInfo.activityInfo;
        if (appOpsManager.noteOpNoThrow(i, activityInfo.applicationInfo.uid, activityInfo.packageName, str, "Broadcast delivered to " + resolveInfo.activityInfo.name) == 0) {
            return true;
        }
    ...
    private boolean requestStartTargetPermissionsReviewIfNeededLocked(BroadcastRecord broadcastRecord, String str, final int i) {
        ...
        if (z && broadcastRecord.intent.getComponent() != null) {
            ...
            if (ActivityManagerDebugConfig.DEBUG_PERMISSIONS_REVIEW) {
                Slog.i(BroadcastQueue.TAG, "u" + i + " Launching permission review for package " + str);
            }
