...
public final class PendingIntentRecord extends IIntentSender.Stub {
    ...
    private IPendingIntentRecordExt mPendingIntentRecordExt = (IPendingIntentRecordExt) ExtLoader.type(IPendingIntentRecordExt.class).base(this).create();
    ...
    private static BackgroundStartPrivileges getBackgroundStartPrivilegesAllowedByCaller(Bundle bundle, int i, String str) {
    ...
    public static BackgroundStartPrivileges getDefaultBackgroundStartPrivileges(int i, String str) {
        ...
        if (str != null) {
            isChangeEnabled = CompatChanges.isChangeEnabled(DEFAULT_RESCIND_BAL_PRIVILEGES_FROM_PENDING_INTENT_SENDER, str, UserHandle.getUserHandleForUid(i));
        } else {
            isChangeEnabled = CompatChanges.isChangeEnabled(DEFAULT_RESCIND_BAL_PRIVILEGES_FROM_PENDING_INTENT_SENDER, i);
        }
        if (isChangeEnabled) {
    ...
    public int sendInner(IApplicationThread iApplicationThread, int i, Intent intent, String str, IBinder iBinder, IIntentReceiver iIntentReceiver, String str2, IBinder iBinder2, String str3, int i2, int i3, int i4, Bundle bundle) {
        ...
        synchronized (this.controller.mLock) {
            if (ActivityManagerDebugConfig.DEBUG_PROCESSES) {
                Slog.d("ActivityManager", "sendInner is canceled is " + this.canceled + ", key's type is " + this.key.type);
            }
            if (this.canceled) {
                return -96;
            }
            ...
            this.mPendingIntentRecordExt.markFinalIntentBeforeCall(intent2, intent, callingUid);
            if (this.key.type == 1) {
            ...
            try {
                ...
                int i10 = i9;
                if (ActivityManagerDebugConfig.DEBUG_PROCESSES) {
                    Slog.d("ActivityManager", "sendInner key's type is " + this.key.type);
                }
                Key key3 = this.key;
                int i11 = key3.type;
                if (i11 == 1) {
                ...
                try {
                    if (intentArr2 != null && intentArr2.length > 1) {
                        ...
                        intent = intent2;
                        i5 = 0;
                        startActivityInPackage = this.controller.mAtmInternal.startActivityInPackage(this.uid, callingPid, callingUid, key3.packageName, key3.featureId, intent, str5, iBinder2, str3, i2, 0, safeActivityOptions2, i10, (Task) null, "PendingIntentRecord", false, this, getBackgroundStartPrivilegesForActivitySender(iBinder));
                    }
