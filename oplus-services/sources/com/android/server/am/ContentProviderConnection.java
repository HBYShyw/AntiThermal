...
public final class ContentProviderConnection extends Binder {
        ...
        if (this.association == null) {
            ...
            if (contentProviderRecord.proc != null) {
                ...
                if (processStateHolder == null) {
                    Slog.wtf(IActivityManagerServiceExt.TAG, "No package in referenced provider " + this.provider.name.toShortString() + ": proc=" + this.provider.proc);
                    return;
                }
                if (processStateHolder.pkg == null) {
                    Slog.wtf(IActivityManagerServiceExt.TAG, "Inactive holder in referenced provider " + this.provider.name.toShortString() + ": proc=" + this.provider.proc);
                    return;
                }
    ...
    public int incrementCount(boolean z) {
        ...
        synchronized (this.mLock) {
            if (ActivityManagerDebugConfig.DEBUG_PROVIDER) {
                ContentProviderRecord contentProviderRecord = this.provider;
                Slog.v(IActivityManagerServiceExt.TAG, "Adding provider requested by " + this.client.processName + " from process " + contentProviderRecord.info.processName + ": " + contentProviderRecord.name.flattenToShortString() + " scnt=" + this.mStableCount + " uscnt=" + this.mUnstableCount);
            }
            if (z) {
                this.mStableCount++;
                this.mNumStableIncs++;
    ...
    public int decrementCount(boolean z) {
        ...
        synchronized (this.mLock) {
            if (ActivityManagerDebugConfig.DEBUG_PROVIDER) {
                ContentProviderRecord contentProviderRecord = this.provider;
                Slog.v(IActivityManagerServiceExt.TAG, "Removing provider requested by " + this.client.processName + " from process " + contentProviderRecord.info.processName + ": " + contentProviderRecord.name.flattenToShortString() + " scnt=" + this.mStableCount + " uscnt=" + this.mUnstableCount);
            }
            if (z) {
                this.mStableCount--;
            } else {
