...
public final class ProcessStatsService extends IProcessStats.Stub {
    ...
    @GuardedBy({"mFileLock"})
    private boolean readLF(ProcessStats processStats, AtomicFile atomicFile) {
        try {
    ...
    @GuardedBy({"mFileLock"})
    private ArrayList<String> getCommittedFilesLF(int i, boolean z, boolean z2) {
    ...
    @GuardedBy({"mLock"})
    private boolean dumpFilteredProcessesCsvLocked(PrintWriter printWriter, String str, boolean z, int[] iArr, boolean z2, int[] iArr2, boolean z3, int[] iArr3, long j, String str2) {
    ...
    static int parseSectionOptions(String str) {
            ...
            try {
                ...
                if (committedFilesLF != null) {
                    for (int size = committedFilesLF.size() - 1; size >= 0; size--) {
                        try {
                            list.add(ParcelFileDescriptor.open(new File(committedFilesLF.get(size)), AudioFormat.EVRC));
                        } catch (IOException e) {
                            Slog.w(TAG, "Failure opening procstat file " + committedFilesLF.get(size), e);
                        }
                    }
                }
            } catch (Throwable th) {
        ...
        try {
            ...
            try {
                ...
                for (int size = committedFilesLF.size() - 1; size >= 0; size--) {
                    ...
                    try {
                        if (str.substring(str.lastIndexOf(STATE_FILE_PREFIX) + 9, str.lastIndexOf(STATE_FILE_SUFFIX)).compareToIgnoreCase(charSequence) > 0) {
                            ParcelFileDescriptor.AutoCloseInputStream autoCloseInputStream = new ParcelFileDescriptor.AutoCloseInputStream(ParcelFileDescriptor.open(new File(str), AudioFormat.EVRC));
                            ...
                            if (processStats2.mReadError != null) {
                                Log.w(TAG, "Failure reading process stats: " + processStats2.mReadError);
                            }
                        }
                    } catch (IOException e2) {
                        Slog.w(TAG, "Failure opening procstat file " + str, e2);
                    } catch (IndexOutOfBoundsException e3) {
                        Slog.w(TAG, "Failure to read and parse commit file " + str, e3);
                    }
                }
                if (z && list != null) {
    ...
    private static ParcelFileDescriptor protoToParcelFileDescriptor(final ProcessStats processStats, final int i) throws IOException {
                    ...
                    if (committedFilesLF != null && committedFilesLF.size() > 0) {
                        ...
                        while (size >= 0 && processStats3.mTimePeriodEndRealtime - processStats3.mTimePeriodStartRealtime < j) {
                            ...
                            if (processStats4.mReadError == null) {
                                ...
                                Slog.w(TAG, "Failure reading " + committedFilesLF.get(size + 1) + "; " + processStats4.mReadError);
                            }
                        }
    ...
    private void dumpAggregatedStats(PrintWriter printWriter, long j, long j2, String str, boolean z, boolean z2, boolean z3, boolean z4, boolean z5, int i) {
    ...
    */
    private void dumpInner(PrintWriter printWriter, String[] strArr) {
        int[] iArr;
        int i;
        int i2;
        String str;
                        ...
                        if ("--csv-mem".equals(str4)) {
                            ...
                            if ("--clear".equals(str4)) {
                                synchronized (processStatsService.mLock) {
                                    ...
                                    try {
                                        ArrayList<String> committedFilesLF = processStatsService.getCommittedFilesLF(0, i10, i10);
                                        if (committedFilesLF != null) {
        ...
        if (i4 > 0) {
            ...
            try {
                ...
                AtomicFile atomicFile = new AtomicFile(new File(committedFilesLF2.get(i4)));
                ProcessStats processStats = new ProcessStats(false);
                ...
                if (processStats.mReadError != null) {
                    ...
                    printWriter.print("Failure reading ");
                    printWriter.print(committedFilesLF2.get(i4));
                    printWriter.print("; ");
                    printWriter.println(processStats.mReadError);
                    return;
                }
        ...
        if ((z9 || z11) && !z12) {
            processStatsService.mFileLock.lock();
            try {
                ArrayList<String> committedFilesLF3 = processStatsService.getCommittedFilesLF(0, false, !z11);
                if (committedFilesLF3 != null) {
                    ...
                    while (i18 < committedFilesLF3.size()) {
                        try {
                            AtomicFile atomicFile2 = new AtomicFile(new File(committedFilesLF3.get(i18)));
                            try {
                                ProcessStats processStats2 = new ProcessStats(false);
                                ...
                                if (processStats2.mReadError != null) {
                                    ...
                                    printWriter.print("Failure reading ");
                                    printWriter.print(committedFilesLF3.get(i18));
                                    printWriter.print("; ");
                                    printWriter.println(processStats2.mReadError);
                                    new File(committedFilesLF3.get(i18)).delete();
                                    i6 = i18;
    ...
    private void dumpProtoForStatsd(FileDescriptor fileDescriptor) {
