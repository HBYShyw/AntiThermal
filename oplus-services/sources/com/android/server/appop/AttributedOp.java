...
public final class AttributedOp {
    ...
    private void startedOrPaused(IBinder iBinder, int i, String str, String str2, int i2, int i3, boolean z, boolean z2, int i4, int i5) throws RemoteException {
        ...
        if (z && !this.parent.isRunning() && z2) {
            ...
            AppOpsService.Op op = this.parent;
            appOpsService.scheduleOpActiveChangedIfNeededLocked(op.op, op.uid, op.packageName, this.tag, true, i4, i5);
        }
        ...
        if (z2) {
            ...
            AppOpsService.Op op2 = attributedOp.parent;
            historicalRegistry.incrementOpAccessedCount(op2.op, op2.uid, op2.packageName, attributedOp.tag, i2, i3, currentTimeMillis, i4, i5);
        }
    }
    ...
    private void finishOrPause(IBinder iBinder, boolean z, boolean z2) {
        ...
        if (valueAt.mNumUnfinishedStarts == 0 || z2) {
            ...
            HistoricalRegistry historicalRegistry = this.mAppOpsService.mHistoricalRegistry;
            AppOpsService.Op op = this.parent;
            historicalRegistry.increaseOpAccessDuration(op.op, op.uid, op.packageName, this.tag, valueAt.getUidState(), valueAt.getFlags(), noteOpEvent.getNoteTime(), noteOpEvent.getDuration(), valueAt.getAttributionFlags(), valueAt.getAttributionChainId());
            if (z2) {
                return;
            }
            ...
            if (this.mInProgressEvents.isEmpty()) {
                ...
                appOpsService.scheduleOpActiveChangedIfNeededLocked(op2.op, op2.uid, op2.packageName, this.tag, false, valueAt.getAttributionFlags(), valueAt.getAttributionChainId());
            }
        }
    }
    ...
    public void pause() {
        if (isRunning()) {
            ...
            for (int i = 0; i < this.mInProgressEvents.size(); i++) {
                ...
                AppOpsService appOpsService = this.mAppOpsService;
                AppOpsService.Op op = this.parent;
                appOpsService.scheduleOpActiveChangedIfNeededLocked(op.op, op.uid, op.packageName, this.tag, false, valueAt.getAttributionFlags(), valueAt.getAttributionChainId());
            }
            this.mInProgressEvents = null;
        }
    }
    ...
    public void resume() {
        if (isPaused()) {
            ...
            for (int i = 0; i < this.mPausedInProgressEvents.size(); i++) {
                ...
                HistoricalRegistry historicalRegistry = this.mAppOpsService.mHistoricalRegistry;
                AppOpsService.Op op = this.parent;
                historicalRegistry.incrementOpAccessedCount(op.op, op.uid, op.packageName, this.tag, valueAt.getUidState(), valueAt.getFlags(), currentTimeMillis, valueAt.getAttributionFlags(), valueAt.getAttributionChainId());
                if (z) {
                    AppOpsService appOpsService = this.mAppOpsService;
                    AppOpsService.Op op2 = this.parent;
                    appOpsService.scheduleOpActiveChangedIfNeededLocked(op2.op, op2.uid, op2.packageName, this.tag, true, valueAt.getAttributionFlags(), valueAt.getAttributionChainId());
                }
                AppOpsService appOpsService2 = this.mAppOpsService;
                AppOpsService.Op op3 = this.parent;
                appOpsService2.scheduleOpStartedIfNeededLocked(op3.op, op3.uid, op3.packageName, this.tag, valueAt.getFlags(), 0, 2, valueAt.getAttributionFlags(), valueAt.getAttributionChainId());
            }
            this.mPausedInProgressEvents = null;
        }
    }
