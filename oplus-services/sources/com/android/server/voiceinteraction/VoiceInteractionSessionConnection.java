...
public final class VoiceInteractionSessionConnection implements ServiceConnection, AssistDataRequester.AssistDataRequesterCallbacks {
    ...
    public class PowerBoostSetter implements Runnable {
        ...
        PowerBoostSetter(Instant instant) {
        ...
        public void run() {
            synchronized (VoiceInteractionSessionConnection.this.mLock) {
                ...
                if (Instant.now().isBefore(this.mExpiryTime)) {
                    VoiceInteractionSessionConnection.this.mPowerManagerInternal.setPowerBoost(0, VoiceInteractionSessionConnection.BOOST_TIMEOUT_MS);
    ...
    public VoiceInteractionSessionConnection(Object obj, ComponentName componentName, int i, Context context, Callback callback, int i2, Handler handler) {
        ...
        Slog.w(TAG, "Failed binding to voice interaction session service " + componentName);
    }
    ...
    private void doHandleAssistWithoutData(List<ActivityAssistInfo> list) {
        int size = list.size();
    ...
    private void handleVisibleActivitiesLocked(IBinder iBinder, int i) {
        ...
        if (this.mListeningVisibleActivity && this.mShown && !this.mCanceled && this.mSession != null) {
            if (i != 1 && i != 2) {
                ...
                if (i == 3) {
                    ...
                    Slog.w(TAG, "notifyActivityEventChangedLocked unexpected type=" + i);
                    return;
                }
    ...
    private void notifyVisibleActivitiesChangedLocked(ArrayMap<IBinder, VisibleActivityInfo> arrayMap, int i) {
    ...
    private VisibleActivityInfo getVisibleActivityInfoFromTopVisibleActivity(IBinder iBinder) {
        ArrayMap<IBinder, VisibleActivityInfo> topVisibleActivityInfosLocked = getTopVisibleActivityInfosLocked();
