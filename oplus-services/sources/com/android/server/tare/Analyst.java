...
public class Analyst {
    ...
    public void noteBatteryLevelChange(int i) {
        ...
        if ((report != null && i >= 90 && report.currentBatteryLevel < i && report.cumulativeBatteryDischarge >= 25) || (report != null && report.currentBatteryLevel < i && (report.screenOffDurationMs > 86400000L ? 1 : (report.screenOffDurationMs == 86400000L ? 0 : -1)) >= 0)) {
            int i3 = (i2 + 1) % 8;
    ...
    public void noteTransaction(Ledger.Transaction transaction) {
        ...
        if (eventType != 1073741824) {
            return;
        }
        long j3 = transaction.delta;
        long j4 = -j3;
        long j5 = transaction.ctp;
        if (j4 > j5) {
            report.cumulativeProfit += (-j3) - j5;
            report.numProfitableActions++;
        } else if ((-j3) < j5) {
            report.cumulativeLoss += j5 + j3;
