...
public class XmlParser {
    public static DeviceStateConfig read(InputStream inputStream) throws XmlPullParserException, IOException, DatatypeConfigurationException {
        XmlPullParser newPullParser = XmlPullParserFactory.newInstance().newPullParser();
        newPullParser.setFeature("http://xmlpull.org/v1/doc/features.html#process-namespaces", true);
        newPullParser.setInput(inputStream, null);
        newPullParser.nextTag();
        if (newPullParser.getName().equals("device-state-config")) {
            return DeviceStateConfig.read(newPullParser);
        }
        return null;
    }
