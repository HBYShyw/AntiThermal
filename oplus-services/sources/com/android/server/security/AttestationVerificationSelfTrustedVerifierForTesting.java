...
class AttestationVerificationSelfTrustedVerifierForTesting {
    ...
    private boolean verifyCertificateChain(List<X509Certificate> list) {
        if (list.size() < 2) {
    ...
    private boolean validateRequirements(Bundle bundle) {
        if (bundle.size() != 1) {
    ...
    private boolean checkLeafChallenge(Bundle bundle, List<X509Certificate> list) {
        try {
    ...
    private byte[] getChallengeFromCert(X509Certificate x509Certificate) throws CertificateEncodingException, IOException {
        return Certificate.getInstance(new ASN1InputStream(x509Certificate.getEncoded()).readObject()).getTBSCertificate().getExtensions().getExtensionParsedValue(new ASN1ObjectIdentifier(ANDROID_KEYMINT_KEY_DESCRIPTION_EXTENSION_OID)).getObjectAt(4).getOctets();
    }
}
