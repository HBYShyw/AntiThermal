...
public class ContextHubEventLogger {
    ...
    private static ContextHubEventLogger sInstance;
    private final ConcurrentLinkedEvictingDeque<NanoappLoadEvent> mNanoappLoadEventQueue = new ConcurrentLinkedEvictingDeque<>(20);
    private final ConcurrentLinkedEvictingDeque<NanoappUnloadEvent> mNanoappUnloadEventQueue = new ConcurrentLinkedEvictingDeque<>(20);
    private final ConcurrentLinkedEvictingDeque<NanoappMessageEvent> mMessageFromNanoappQueue = new ConcurrentLinkedEvictingDeque<>(20);
    private final ConcurrentLinkedEvictingDeque<NanoappMessageEvent> mMessageToNanoappQueue = new ConcurrentLinkedEvictingDeque<>(20);
    private final ConcurrentLinkedEvictingDeque<ContextHubRestartEvent> mContextHubRestartEventQueue = new ConcurrentLinkedEvictingDeque<>(20);
    ...
    public synchronized void clear() {
        Collection[] collectionArr = {this.mNanoappLoadEventQueue, this.mNanoappUnloadEventQueue, this.mMessageFromNanoappQueue, this.mMessageToNanoappQueue, this.mContextHubRestartEventQueue};
        for (int i = 0; i < 5; i++) {
    ...
    public synchronized void logNanoappLoad(int i, long j, int i2, long j2, boolean z) {
        ...
        if (!this.mNanoappLoadEventQueue.add(nanoappLoadEvent)) {
            Log.e(TAG, "Unable to add nanoapp load event to queue: " + nanoappLoadEvent);
        }
    }
    ...
    public synchronized void logNanoappUnload(int i, long j, boolean z) {
        ...
        if (!this.mNanoappUnloadEventQueue.add(nanoappUnloadEvent)) {
            Log.e(TAG, "Unable to add nanoapp unload event to queue: " + nanoappUnloadEvent);
        }
    }
    ...
    public synchronized void logMessageFromNanoapp(int i, NanoAppMessage nanoAppMessage, boolean z) {
        ...
        if (!this.mMessageFromNanoappQueue.add(nanoappMessageEvent)) {
            Log.e(TAG, "Unable to add message from nanoapp event to queue: " + nanoappMessageEvent);
        }
    }
    ...
    public synchronized void logMessageToNanoapp(int i, NanoAppMessage nanoAppMessage, boolean z) {
        ...
        if (!this.mMessageToNanoappQueue.add(nanoappMessageEvent)) {
            Log.e(TAG, "Unable to add message to nanoapp event to queue: " + nanoappMessageEvent);
        }
    }
    ...
    public synchronized void logContextHubRestart(int i) {
        ...
        if (!this.mContextHubRestartEventQueue.add(contextHubRestartEvent)) {
            Log.e(TAG, "Unable to add Context Hub restart event to queue: " + contextHubRestartEvent);
        }
    }
