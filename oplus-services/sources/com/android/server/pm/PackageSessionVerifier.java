...
public final class PackageSessionVerifier {
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public void verifyStaged(final StagingManager.StagedSession stagedSession, final Callback callback) {
    ...
    private void dispatchVerifyApex(final StagingManager.StagedSession stagedSession, final Callback callback) {
    ...
    private void dispatchEndVerification(final StagingManager.StagedSession stagedSession, final Callback callback) {
    ...
    private void verifyApex(StagingManager.StagedSession stagedSession) throws PackageManagerException {
        int i = -1;
        if ((stagedSession.sessionParams().installFlags & DumpState.DUMP_DOMAIN_PREFERRED) != 0) {
            try {
    ...
    private boolean isApexUpdateAllowed(String str, String str2) {
        ...
        String str3 = (String) SystemConfig.getInstance().getAllowedVendorApexes().get(str);
        if (str3 == null) {
    ...
    void checkOverlaps(StagingManager.StagedSession stagedSession, StagingManager.StagedSession stagedSession2) throws PackageManagerException {
        ...
        for (StagingManager.StagedSession stagedSession3 : this.mStagedSessions) {
            if (!stagedSession3.isDestroyed() && !stagedSession3.isInTerminalState() && stagedSession3.sessionId() != stagedSession.sessionId() && stagedSession3.sessionContains(new Predicate() { // from class: com.android.server.pm.PackageSessionVerifier$$ExternalSyntheticLambda4
                @Override // java.util.function.Predicate
                public final boolean test(Object obj) {
