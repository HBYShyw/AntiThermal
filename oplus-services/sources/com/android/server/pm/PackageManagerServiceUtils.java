...
public class PackageManagerServiceUtils {
    ...
    public static final Predicate<PackageStateInternal> REMOVE_IF_APEX_PKG = new Predicate() { // from class: com.android.server.pm.PackageManagerServiceUtils$$ExternalSyntheticLambda1
    ...
    public static final ThreadLocal<Boolean> DISABLE_ENFORCE_INTENTS_TO_MATCH_INTENT_FILTERS = ThreadLocal.withInitial(new Supplier() { // from class: com.android.server.pm.PackageManagerServiceUtils$$ExternalSyntheticLambda3
    ...
    public static boolean checkISA(String str) {
        if (Build.MTK_HBT_ON_64BIT_ONLY_CHIP) {
            for (String str2 : Build.MTK_HBT_SUPPORTED_ABIS) {
                if (VMRuntime.getInstructionSet(str2).equals(str)) {
                    return true;
                }
            }
        } else if (Build.QCOM_TANGO_ON_64BIT_ONLY_CHIP) {
            for (String str3 : Build.QCOM_TANGO_SUPPORTED_ABIS) {
                if (VMRuntime.getInstructionSet(str3).equals(str)) {
                    return true;
                }
            }
        } else {
            for (String str4 : Build.SUPPORTED_ABIS) {
                if (VMRuntime.getInstructionSet(str4).equals(str)) {
                    return true;
                }
            }
        }
    ...
    private static boolean matchSignaturesCompat(String str, PackageSignatures packageSignatures, SigningDetails signingDetails) {
        ArraySet arraySet = new ArraySet();
    ...
    private static boolean matchSignaturesRecover(String str, SigningDetails signingDetails, SigningDetails signingDetails2, @SigningDetails.CertCapabilities int i) {
        String message;
        try {
    ...
    private static boolean matchSignatureInSystem(String str, SigningDetails signingDetails, PackageSetting packageSetting) {
    ...
    public static int extractNativeBinaries(File file, String str) {
        ...
        try {
            try {
                AutoCloseable create = NativeLibraryHelper.Handle.create(file);
                try {
    ...
    public static int decompressFiles(String str, File file, String str2) {
        ...
        try {
            ...
            for (File file2 : compressedFiles) {
                try {
                    String substring = file2.getName().substring(0, r8.length() - 3);
    ...
    private static void copyFile(String str, File file, String str2) throws ErrnoException, IOException {
        ...
        FileDescriptor open = Os.open(file2.getAbsolutePath(), OsConstants.O_RDWR | OsConstants.O_CREAT, 420);
    ...
    public static void applyEnforceIntentFilterMatching(PlatformCompat platformCompat, ComponentResolverApi componentResolverApi, List<ResolveInfo> list, boolean z, Intent intent, String str, int i) {
        ...
        if (DISABLE_ENFORCE_INTENTS_TO_MATCH_INTENT_FILTERS.get().booleanValue() || (appId = UserHandle.getAppId(i)) == 0 || appId == 1000) {
            return;
        }
        String str2 = null;
        final LogPrinter logPrinter = PackageManagerService.DEBUG_INTENT_MATCHING ? new LogPrinter(2, "PackageManager", 3) : null;
        int size = list.size() - 1;
        while (size >= 0) {
            ComponentInfo componentInfo = list.get(size).getComponentInfo();
            boolean z2 = false;
            if (ActivityManager.checkComponentPermission(str2, i, componentInfo.applicationInfo.uid, false) != 0) {
                ...
                if (service != null && !service.getIntents().isEmpty()) {
                    boolean isChangeEnabledInternal = platformCompat.isChangeEnabledInternal(ENFORCE_INTENTS_TO_MATCH_INTENT_FILTERS, componentInfo.applicationInfo);
                    ...
                    while (true) {
                        ...
                        if (IntentResolver.intentMatchesFilter(service.getIntents().get(i2).getIntentFilter(), intent, str)) {
                            z2 = true;
                            break;
                        }
                        i2++;
                    ...
                    if (!z2) {
                        ...
                        if (isChangeEnabledInternal) {
                            ...
                            if (PackageManagerService.DEBUG_INTENT_MATCHING) {
                                ...
                                service.getIntents().forEach(new Consumer() { // from class: com.android.server.pm.PackageManagerServiceUtils$$ExternalSyntheticLambda0
                                    @Override // java.util.function.Consumer
                                    public final void accept(Object obj) {
                                        PackageManagerServiceUtils.lambda$applyEnforceIntentFilterMatching$3(logPrinter, (ParsedIntentInfo) obj);
                                    }
                                });
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public static /* synthetic */ void lambda$applyEnforceIntentFilterMatching$3(Printer printer, ParsedIntentInfo parsedIntentInfo) {
        parsedIntentInfo.getIntentFilter().dump(printer, "  ");
    }
    ...
    static String tryParsePackageName(String str) throws IllegalArgumentException {
    ...
    public static boolean isInstalledByAdb(String str) {
        return str == null || VibratorManagerService.VibratorManagerShellCommand.SHELL_PACKAGE_NAME.equals(str);
    }
}
