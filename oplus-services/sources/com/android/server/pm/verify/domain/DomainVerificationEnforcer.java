...
public class DomainVerificationEnforcer {
    ...
    public void assertApprovedQuerent(int i, DomainVerificationProxy domainVerificationProxy) {
        ...
        if (!domainVerificationProxy.isCallerVerifier(i)) {
            this.mContext.enforcePermission("android.permission.DUMP", Binder.getCallingPid(), i, "Caller " + i + " is not allowed to query domain verification state");
            return;
        }
        this.mContext.enforcePermission("android.permission.QUERY_ALL_PACKAGES", Binder.getCallingPid(), i, "Caller " + i + " does not hold android.permission.QUERY_ALL_PACKAGES");
    }
