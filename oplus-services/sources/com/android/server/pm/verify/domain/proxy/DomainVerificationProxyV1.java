...
public class DomainVerificationProxyV1 implements DomainVerificationProxy {
        ...
        if (!arraySet2.isEmpty()) {
            try {
                if (this.mManager.setDomainVerificationStatusInternal(i3, uuid, arraySet2, 1) != 0) {
                    Slog.e(TAG, "Failure reporting successful domains for " + str2);
                }
            } catch (Exception e) {
                Slog.e(TAG, "Failure reporting successful domains for " + str2, e);
            }
        }
        if (!arraySet.isEmpty()) {
            try {
                if (this.mManager.setDomainVerificationStatusInternal(i3, uuid, arraySet, 6) != 0) {
                    Slog.e(TAG, "Failure reporting failed domains for " + str2);
                }
            } catch (Exception e2) {
                Slog.e(TAG, "Failure reporting failed domains for " + str2, e2);
            }
        }
        return true;
    }
    ...
    private void sendBroadcasts(ArrayMap<Integer, Pair<UUID, String>> arrayMap) {
        long powerSaveTempWhitelistAppDuration = this.mConnection.getPowerSaveTempWhitelistAppDuration();
        this.mConnection.getDeviceIdleInternal().addPowerSaveTempWhitelistApp(Process.myUid(), this.mVerifierComponent.getPackageName(), powerSaveTempWhitelistAppDuration, 0, true, 307, "domain verification agent");
        ...
        for (int i = 0; i < size; i++) {
            ...
            makeBasic.setTemporaryAppAllowlist(powerSaveTempWhitelistAppDuration, 0, 307, "");
            this.mContext.sendBroadcastAsUser(addFlags, UserHandle.SYSTEM, null, makeBasic.toBundle());
        }
    }

    private String buildHostsString(AndroidPackage androidPackage) {
    ...
    private static class Response {
        ...
        private Response(int i, int i2, int i3, List<String> list) {
            this.callingUid = i;
